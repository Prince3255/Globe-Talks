...
import { useGetEntertainmentQuery } from '../Redux/NewsApi/GoogleNews'
...
function Entertainment() {
  ...
  const handleMouseEnter = () => {
    ...
    if (!hover) {
      dispatch(NewsApi.util.prefetch('getEntertainment', undefined, {force: true}));
      setHover(true)
    }
  }

  const { data, isLoading, error } = useGetEntertainmentQuery(language)
  ...
  return (
    <div onMouseEnter={handleMouseEnter} className={`w-full min-h-screen mt-60 md:mt-40 mb-5 ${isLoading ? animate-pulse : ''}`}>
      <h1 className='text-center text-2xl font-serif my-10'>Entertainment Spotlight</h1>
      {
