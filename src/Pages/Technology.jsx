...
import { useGetTechnologyQuery } from '../Redux/NewsApi/GoogleNews'
...
function Technology() {
  ...
  const handleMouseEnter = () => {
    ...
    if (!hover) {
      dispatch(NewsApi.util.prefetch('getTechnology', undefined, {force: true}));
      setHover(true)
    }
  }

  const { data, isLoading, error } = useGetTechnologyQuery(language)
  ...
  return (
    <div onMouseEnter={handleMouseEnter} className={`w-full min-h-screen mt-60 md:mt-40 mb-5 ${isLoading ? animate-pulse : ''}`}>
      <h1 className='text-center text-2xl font-serif my-10'>Tech News</h1>
      {
