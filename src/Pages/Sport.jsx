...
import { useGetSportQuery } from '../Redux/NewsApi/GoogleNews'
...
function Sport() {
  ...
  const handleMouseEnter = () => {
    ...
    if (!hover) {
      dispatch(NewsApi.util.prefetch('getSport', undefined, {force: true}));
      setHover(true)
    }
  }

  const { data, isLoading, error } = useGetSportQuery(language)
  ...
  return (
    <div onMouseEnter={handleMouseEnter} className={`w-full min-h-screen mt-60 md:mt-40 mb-5 ${isLoading ? animate-pulse : ''}`}>
      <h1 className='text-center text-2xl font-serif my-10'>Latest Sports Updates</h1>
      {
