...
import { useGetWorldQuery } from '../Redux/NewsApi/GoogleNews'
...
function World() {
  ...
  const handleMouseEnter = () => {
    ...
    if (!hover) {
      dispatch(NewsApi.util.prefetch('getWorld', undefined, {force: true}));
      setHover(true)
    }
  }

  const { data, isLoading, error } = useGetWorldQuery(language)

  if (isLoading) return <Loader />

  if (error) return <Error />

  if (!data) return <div className='text-2xl text-center text-black mt-40 mb-5'>No results found</div>;
  ...
  return (
    <div onMouseEnter={handleMouseEnter} className={`w-full min-h-screen mt-60 md:mt-40 mb-5 ${isLoading ? animate-pulse : ''}`}>
      <h1 className='text-center text-2xl font-serif my-10'>Global News Updates</h1>
      {
